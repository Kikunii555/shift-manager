...
const DEFAULT_TASKS_TEMPLATE = [
  ...
  },
  {
    ...
    subtasks: [
      ...
      { text: "ファイルを開いてマクロ2か所を有効にし、\n監督社印と社員確認欄を削除する" },
      { text: "左上の職場名を修正" },
      { text: "【正社員】祝日代休と限定非番を追加" },
      { text: "【正社員】局長欄と局長押印欄を追加\n（押印欄の幅は60px）" },
      { text: "印刷設定で用紙サイズをA3の手差し印刷に変更し、\n印刷範囲を\"すべての行を1ページに印刷する\"に設定" },
      { text: "【正社員】いったん戻って余白の幅を8pxに変更" },
      { text: "【正社員】内容があっているか確認し、A3で印刷" },
      { text: "【期間雇用社員】内容があっているか確認し、A3で印刷" },
      { text: "業務支援の確定を解除" },
  ...
  // 4. 提出～認証
  { categoryId: 4, text: "計画に報告して提出", subtasks: [] },
  {
    categoryId: 4,
    text: "勤務指定表の再作成",
    isBranch: true,
  ...
  { categoryId: 4, text: "中身を確認後、局長へ提出", subtasks: [] },

  // 5. 仕上げと次の準備
  {
    categoryId: 5, text: "出勤簿に服務スタンプを押印",
    subtasks: [
      { text: "週休" },
      { text: "非番" },
      { text: "限定非番" },
  ...
  { categoryId: 5, text: "局長押印を確認後に掲示", subtasks: [] },
  { categoryId: 5, text: "計年計画一覧を修正・入力", subtasks: [] },
];
      ...
      state.periods.forEach(period => {
        ...
        if (hasLegacyTasks || hasNoCategory3 || isTaskCountMismatch) {
          ...
          period.tasks = DEFAULT_TASKS_TEMPLATE.map((t, idx) => {
            ...
            if (t.isBranch) {
              taskObj.isBranch = true;
              taskObj.selectedOption = null;
              taskObj.branchOptions = t.branchOptions;
            }
            return taskObj;
          });
        }
  ...
  CATEGORIES.forEach(category => {
    ...
    categoryTasks.forEach(task => {
      ...
      if (task.isBranch) {
        ...
        branchSelector.style.marginTop = '4px';
        branchSelector.style.marginBottom = '4px';
        branchSelector.style.paddingLeft = '32px';

        const options = task.branchOptions || [
          { key: "no_changes", label: "修正箇所がない" },
          { key: "has_changes", label: "修正箇所がある" }
        ];

        options.forEach(opt => {
          const btn = document.createElement('button');
          btn.className = 'branch-opt-btn';
          ...
          const isSelected = task.selectedOption === opt.key;
          if (isSelected) {
          ...
          btn.addEventListener('click', (e) => {
            e.stopPropagation();
            setBranchOption(period.id, task.id, opt.key);
          });
      ...
      if (branchSelector) {
        item.appendChild(branchSelector);
      }
...
// 分岐オプションの選択
function setBranchOption(periodId, taskId, optionKey) {
  const period = state.periods.find(p => p.id === periodId);
  if (period) {
    const task = period.tasks.find(t => t.id === taskId);
    if (task && task.isBranch) {
      if (task.selectedOption === optionKey) return;
      task.selectedOption = optionKey;

      const template = DEFAULT_TASKS_TEMPLATE.find(t => t.text === task.text);
      if (template && template.branchOptions) {
        const option = template.branchOptions.find(o => o.key === optionKey);
        if (option) {
          const baseTime = Date.now();
          task.subtasks = option.subtasks.map((st, sIdx) => {
            return {
              id: baseTime + sIdx + 2000,
              text: st.text,
              checked: false,
              checkedAt: null
            };
